(function ()
{
    ...
    function seedUsers(users)
    {
        ...
        userSchema = userSchema || mongoose.model('user', require('../app/dao/userDAO.service.js').schema);
        return userSchema.createQ(users);
    }

    function openDBConnection()
    {
        mongoose.connect('mongodb://localhost/javascript-world', function (error)
        {
            if (error) {
                console.log(error);
            }
    ...
    function messageError(done, expected, result)
    {
    ...
    function isEquals(expected, results, ignore)
    {
        ...
        if (results instanceof Array) {
            for (i = 0; i < results.length; i++) {
                var result = isEquals(expected[i], results[i], ignore);
                if (!result) {
                    return false;
                }
            }
